...
resource "google_project" "shared_operations" {
  name                = "${var.shared_operations_project_name}-${var.tb_discriminator}"
  project_id          = "${var.shared_operations_project_name}-${var.tb_discriminator}"
  folder_id           = var.root_id
  billing_account     = var.billing_account_id
  auto_create_network = false
  depends_on          = [google_project.shared_networking]
...
resource "google_project" "shared_ssp" {
  name                = "${var.shared_ssp_project_name}-${var.tb_discriminator}"
  project_id          = "${var.shared_ssp_project_name}-${var.tb_discriminator}"
  folder_id           = var.root_id
  billing_account     = var.billing_account_id
  auto_create_network = false
  depends_on          = [google_project.shared_networking]
...
resource "google_project" "tb_bastion" {
  name                = "${var.tb_bastion_project_name}-${var.tb_discriminator}"
  project_id          = "${var.tb_bastion_project_name}-${var.tb_discriminator}"
  folder_id           = var.root_id
  billing_account     = var.billing_account_id
  auto_create_network = false
  depends_on          = [google_project.shared_networking]
